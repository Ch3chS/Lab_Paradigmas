...
;La representación estará dada por una lista la cual constara con el formato (int x int x int x int) de la siguiente forma:

;(coordenada x>0, coordenada y>0, bit(0|1), profundidad>0)
...
(define pixbit-d (lambda (x y bit depth)
                   ...
                   (if (pixbit-d? (pixbitcont x y bit depth))
                       ...
                       (list -1 -1 -1 -1)               ;Esta será la representación de un pixbit erroneo
                       )))

;Función que crea una lista a partir de ciertos parametros (idealmente un pixbit-d)
;Dominio: int x int x int x int
;Recorrido: lista
(define pixbitcont (lambda (x y bit depth) (list x y bit depth)))  ;Esta función contenida es donde se construye como tal la representación
...
(define pixbit-d? (lambda (pixbit-d)
                   ...
                   (if (and (list? pixbit-d) (not (equal? pixbit-d (list -1 -1 -1 -1))))
                       ...
                       (if (and (number? (car pixbit-d)) (and (number? (cadr pixbit-d)) (and (number? (caddr pixbit-d)) (number? (cadddr pixbit-d)))))
                           ...
                           (if (and (<= 0 (car pixbit-d)) (and (<= 0 (cadr pixbit-d)) (and (or (= 0 (caddr pixbit-d)) (= 1 (caddr pixbit-d))) (<= 0 (cadddr pixbit-d)))))
                            #t
                            #f
                            )
                           #f
...
(define getpixbit.x (lambda (pixbit-d)
                      (if (pixbit-d? pixbit-d)      ;Comprobamos que la entrada sea un pixbit-d
                          (car pixbit-d)            ;Si lo es, entregamos lo requerido
                         ...
                         (if (pixbit-d? pixbit-d)      ;Comprobamos que la entrada 1 sea un pixbit-d
                             (if (number? x)           ;Comprobamos que la entrada 2 sea un número para no agregar algo que no debe agregarse
                                 (if (<= 0 x)          ;Comprobamos que además este número sea mayor o igual a 0
                                     (list x (getpixbit.y pixbit-d) (getpixbit.bit pixbit-d) (getpixbit.depth pixbit-d))   ;Si todo se cumple retornamos el pixbit-d con el valor cambiado
                                     pixbit-d           
                                 )
                             ...
                             (list -1 -1 -1 -1)     ;Si lo que entro no es un pixbit-d entonces retornamos la representación de pixbit-d erroneo
                         ...
                         (if (pixbit-d? pixbit-d)      ;Comprobamos que la entrada 1 corresponda a un pixbit-d
                             (if (number? bit)         ;Comprobamos que la entrada 2 corresponda a un número
                                 (if (or (= 0 bit) (= 1 bit))  ;Comprobamos que este número sea 0 o 1 (es un bit ciertamente)
                                     (list (getpixbit.x pixbit-d) (getpixbit.y pixbit-d) bit (getpixbit.depth pixbit-d))  ;De cumplirse todo retornamos el pixbit-d con el cambio
                                     pixbit-d           
                                  )
